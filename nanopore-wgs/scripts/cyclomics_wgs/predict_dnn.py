...
class Features:
    def __init__(self, bam_file, add_ref, refseq, readid, hdf5file, chromosome, window = 0):
        self.window = window; 
        x_size = 1134
        ...
        for read in self.bam.fetch():
            # print(read)
            r = str(read).split('\t')
            startpositions.append(int(r[3]))
            readlengths.append(len(r[9]))
        start = min(startpositions)
        length = max(readlengths)

        output_vcf = f'{tmp_bamfolder}/{readid}.vcf'
        with open(output_vcf, 'w') as file:
            file.truncate(0)
            file.write('##fileformat=VCFv4.2'); file.write('\n')
            file.write('#CHROM	POS	ID	REF	ALT'); file.write('\n')
            for pos in range(start - 10, start+length+50):
                base = refseq[pos]
                # if base == 'N': continue
        ...
        self.vcf_df = utils_lambda.read_vcf_pandas(f'{tmp_bamfolder}/{readid}.vcf') 
        # self.vcf_df = None
        # print(self.vcf_df.values)
        ...
        # while i < start + 10000:
            # print(i)
            xmat_perpos, coverage = self.get_mapped_features(i, add_ref, x_size, refseq, start)   
            # print(xmat_perpos.shape, coverage)
            if len(xmat_perpos) == x_size and coverage > 0:
        ...
        if len(self.x_big.shape) == 2:
            self.savehdf5(self.x_big, readid, hdf5file)
        # elif coverage > 2:
        #     self.savehdf5(self.x_big, coverage, readid, hdf5file)                        

    def get_mapped_features(self,i, add_ref, x_size, refseq, start):
        max = 0 
        completerowx = np.zeros((0,)); 
        (chromosome, position) = next(self.variants_iter)[1]
        chromosome_nr = chromosome[3:]
        coverage = 0
        for pileupcolumn in self.bam.pileup(chromosome_nr, int(start)-10, min_base_quality = 0):   
            if pileupcolumn.pos < position + 1 - round(self.window/2): continue
                ...
                if coverage < 3:
                    ...
                    return np.zeros((0,)), coverage   # at this point completerowx is still empty
                if coverage > 20:
                    seq = seq[:20]
                    boqs = boqs[:20]
                    coverage = 20
    ...
    def savehdf5(self, x, readid, hdf5_file):
...
if __name__ == "__main__":
    ...
    bam_file = sys.argv[1]
    txt_file = sys.argv[2]
    tmp_bamfolder = sys.argv[3]
    hdf5_file = sys.argv[4]
    ref_file = sys.argv[5]
    add_ref = 'ref'
    ...
    print(f'tmp bamfolder: {tmp_bamfolder}')
    ...
    for id in ids:
        try:
            ...
            out = pysam.Samfile(f'{tmp_bamfolder}/{id}.bam','wb', header = header)
            for x in iterator: 
                out.write(x)
    ...
    for id in ids:
        try:
            bamfile = f'{tmp_bamfolder}/{id}.bam'
            # pysam.index(bamfile)
            Features(bamfile, add_ref,refseq, id, hdf5_file, chromosome, window=10)
        except Exception as e:     
            print(e)
    ...
    with h5py.File(hdf5_file) as p:
        print(f'File: {hdf5_file}\tNumber of keys: {len(p.keys())}')
    logging.debug("done.")
