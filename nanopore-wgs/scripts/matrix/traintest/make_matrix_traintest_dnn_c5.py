...
class Features:
    def __init__(self, vcf_file, bam_file, add_ref, refseq, hdf5file, window = 0):
        ...
        # while i < 5:
            ...
            i += 1
        self.x_big2 = np.delete(self.x_big, 0, 0)
        self.y_big2 = np.delete(self.y_big, 0, 0)
        print(f'{bam_file}: {self.x_big2.shape}\t{self.y_big2.shape}\tmax aligned: {self.super_max}')
        self.savehdf5(self.x_big2, self.y_big2, hdf5file)    
    ...
    def get_mapped_features(self,i, add_ref, x_size, refseq):
        ...
        for pileupcolumn in self.bam.pileup(chromosome, start, end, min_base_quality = 0):  
            if pileupcolumn.pos < position + 1 - round(self.window/2): continue
                ...
                seq = seq[:coverage]
                if len(seq) < coverage_list[0]: # discard samples with a coverage lower than the minimum coverage specifed in the list
    ...
    def savehdf5(self, x, y, hdf5_file):
