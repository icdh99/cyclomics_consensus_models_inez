...
class Features:
    ...
    def get_mapped_features(self,i, add_ref, x_size, refseq):
        ...
        coverage = 0
        for pileupcolumn in self.bam.pileup('chr17', 7574000, 7579400, min_base_quality = 0):   # REGION 5 = 7579212, 7579351. REGION 2 = 7577658 7577787. REGION 3 = 7577815 7577956. this is region 2 - 5
    ...
    def make_matrix(self, boqs, seq, position, teller, add_ref, x_size, refseq):         # make a list per position of the window with all bases and quality scores
        ...
        if position == vcf[teller,1]: 
            if add_ref == 'ref':
                xmat = np.concatenate((xmat, np.zeros((1, 6)))) 
...
if __name__ == "__main__":
    ...
    with open('/hpc/compgen/projects/gw_cfdna/snv_qs/nanopore-wgs/data/ref_per_chromosome/chr17_v1.1.txt', 'r') as f: 
        lines = [line.rstrip('\n') for line in f]
    ...
    for id in ids:
        try:
            ...
            out = pysam.Samfile(f'tmp_bamfiles_cnn_{region}_{run_id}/{id}.bam','wb', header = header)
    ...
    for id in ids:
        try:
            bamfile = f'tmp_bamfiles_cnn_{region}_{run_id}/{id}.bam'
            pysam.index(bamfile)
    ...
    # paths = [os.path.join('/hpc/compgen/projects/gw_cfdna/snv_qs/nanopore-wgs/data/predict_cyclomics_clean', file) for file in os.listdir('/hpc/compgen/projects/gw_cfdna/snv_qs/nanopore-wgs/data/predict_cyclomics_clean')]
    # for file in paths:
    #     # print(file)
