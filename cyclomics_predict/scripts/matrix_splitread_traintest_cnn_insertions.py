...
class Features:
    def __init__(self, vcf_file, bam_file, consensus_file, cov, add_ref, refseq, readid, hdf5file, window = 0):
        self.window = window; 
        x_size = cov 
        self.bam = utils_lambda.read_bam(bam_file); 
        self.consensus = utils_lambda.read_bam(consensus_file)
        # self.vcf_from_consensus(self.consensus)
        ...
        while i < self.vcf_df[["CHROM", "POS"]].shape[0]:
            xmat_perpos_list, ymat_perpos_list = self.get_mapped_features(i, add_ref, x_size, refseq)   
            for xmat_perpos, ymat_perpos in zip(xmat_perpos_list, ymat_perpos_list):
                # print(xmat_perpos.shape)
                if xmat_perpos.shape == (x_size+1,9,6) and ymat_perpos.shape == (6,):
    ...
    def get_mapped_features(self,i, add_ref, x_size, refseq):
        ...
        listx = [[np.zeros((x_size+1,1,6))] for x in range(5)]         # 5 is zelfgekozen nummer --> aanpassen naar iets groots en dan aan het einde de lege arrays verwijderen!!!!
        ...
        for pileupcolumn in self.bam.pileup(chromosome, min_base_quality = 0):  
            if pileupcolumn.pos < position + 1 - round(self.window/2): continue
                ...
                for t, (list_seq, list_boqs) in enumerate(zip(chunks_seq, chunks_boqs)):
                    ...
                    subx = np.concatenate((subx, xmat), axis=1)
    ...
    def make_matrix(self, boqs, seq, position, teller, add_ref, x_size, refseq):         # make a list per position of the window with all bases and quality scores
        ...
        if position == vcf[teller,1]: 
            ...
            if add_ref == 'ref':
                ...
                xmat = np.concatenate((xmat, self.base_encode(ref_base))) # ref = x 
            elif add_ref ==  'noref':
                xmat = np.concatenate((xmat, self.base_encode(random.choice(["A", "C", "G", "T", "D", "N"],))))
        else:                                               
            if add_ref == 'ref':
                # output = subprocess.getoutput(f'samtools faidx /hpc/compgen/GENOMES/CHM13_t2t/chm13.draft_v1.1.fa chr17:{position-2}-{position-2}')
                # ref_base = output.split()[-1]
    ...
    def savehdf5(self, x, y, x_size, cov, readid, hdf5_file):
        with h5py.File(f'{hdf5_file}_{readid}.hdf5', "a") as f:
            nr_samples = 500
            dx = f.create_dataset('x', data=x, maxshape=(None, x_size+1, 9, 6))
            dy = f.create_dataset('y', data=y, maxshape=(None, 6))
            dx.resize((nr_samples,x_size+1,9,6))
            dy.resize((nr_samples,6))
            # f.create_dataset('x', data=x)
            # f.create_dataset('y', data=y)

if __name__ == "__main__":
    for m, j in enumerate(sys.argv):
        print(m, j)
    vcffile = sys.argv[1]
    bam_file = sys.argv[2]
    ...
    for id in ids:
        ...
        try:
            ...
            out = pysam.Samfile(f'tmp_bamfiles_{run_id}/{id}.bam','wb', header = header)
            for x in iterator:
                out.write(x)
            p += 1
    ...
    for id in ids_working:
        try:
            bamfile = f'tmp_bamfiles_{run_id}/{id}.bam'
            pysam.index(bamfile)
            ...
            Features(vcffile, bamfile,consensusfile, cov,add_ref,refseq,id,hdf5_file,window=10)
        except Exception as e:
            v += 1
            print(e, f'id {id} is not working') 
            print(traceback.format_exc())

    # os.remove('tmp_vcf_current_readid.vcf')  
