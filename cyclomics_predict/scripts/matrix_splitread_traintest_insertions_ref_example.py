...
class Features:
    def __init__(self, vcf_file, bam_file, consensus_file, cov, add_ref, readid, hdf5file, window = 0):
        self.window = window; 
        self.cov = cov
        ...
        while i < self.vcf_df[["CHROM", "POS"]].shape[0]:
            xmat_perpos_list, ymat_perpos_list = self.get_mapped_features(i, add_ref, cov, x_size)   
    ...
    def get_mapped_features(self,i, add_ref, cov, x_size):
        max = 0 
        ...
        for pileupcolumn in self.bam.pileup(chromosome, min_base_quality = 0):  
            if pileupcolumn.pos < position + 1 - round(self.window/2): continue
                ...
                for t, (list_seq, list_boqs) in enumerate(zip(chunks_seq, chunks_boqs)):
                    ...
                    xmat, ymat = self.make_matrix(list_boqs, list_seq, pos_base, i, add_ref, x_size)   # list per position of window,
    ...
    def make_matrix(self, boqs, seq, position, teller, add_ref, x_size):         # make a list per position of the window with all bases and quality scores
...
if __name__ == "__main__":
    ...
    for id in ids_working:
        try:
            ...
            Features(vcffile, bamfile,consensusfile, cov,add_ref,id,hdf5_file,window=10)
        except Exception as e:
            v += 1
