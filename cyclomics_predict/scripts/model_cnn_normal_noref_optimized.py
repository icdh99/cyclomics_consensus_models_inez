...
# READ MERGED HDF5 WITH MUTATIONS, CONCATENATE, REMOVE EMPTY ROWS copied
# with h5py.File(path_allpos) as f:
#     x1 = f['x'][()]
#     y1 = f['y'][()]
#     list_of_arrays_x = []
#     list_of_arrays_y = []
#     for i in range(len(x1)): list_of_arrays_x.append(x1[i])
#     for i in range(len(y1)): list_of_arrays_y.append(y1[i])
#     print(f'{len(list_of_arrays_x)} arrays are concatenated')
#     x_allpos = np.concatenate(list_of_arrays_x, axis = 0)
#     y_allpos = np.concatenate(list_of_arrays_y, axis = 0)
#     nr_emptyrows = len(np.where(~y_allpos.any(axis=1))[0])
#     print(f'number of empty rows that are deleted: {nr_emptyrows}') 
#     x = np.delete(x_allpos, np.where(~y_allpos.any(axis=1))[0], axis = 0) # ~ means not 
#     y = np.delete(y_allpos, np.where(~y_allpos.any(axis=1))[0], axis = 0)
#     print(f'shape of allpos x array: {x_allpos.shape}')
#     print(f'shape of allpos y array: {y_allpos.shape}')
